...
public:
	...
	{
		std::complex<double>coords((double)_xCoord, (double)_yCoord);

		coords = coordinateConversion(screen, frac, coords);

		_escapeIter = escape(coords, 300);

		_scaledEscapeInter = (double)_escapeIter / 300.0;
...
private:
	...
	//Converts a pixel from carteasian to complex coordiantes
	std::complex<double> coordinateConversion(Window<int>& screen, Window<double>& frac, std::complex<double> coords)
	{
		return (coords.real() / (double)screen.getWidth() * frac.getWidth() + frac.getMinX(), coords.imag() / (double)screen.getHeight() * frac.getHeight() + frac.getMinY());
	}
	...
	// Set Colors using bernstein polynomials.
	void color()
	{
	...
	int _xCoord;
	int _yCoord;
	//std::complex<double> _complexCoord;
	int _escapeIter;
